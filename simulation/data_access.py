...
def load_correlation_matrix(w, warehouse_id, channels: list[str]) -> np.ndarray:
    ...
    _, rows = sql(
        w, warehouse_id,
        f"SELECT channel_id_a, channel_id_b, correlation_coefficient FROM {TBL_CORRELATION}",
    )
    ...
    for i, a in enumerate(channels):
        for j, b in enumerate(channels):
            try:
                ...
                raise RuntimeError(
                    f"{TBL_CORRELATION} is missing the ({a}, {b}) pair -- "
